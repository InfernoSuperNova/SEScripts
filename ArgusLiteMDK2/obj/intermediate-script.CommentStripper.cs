...
internal class Program : MyGridProgram
{
    ...
    float MaxAngularVelocityRPM = 30;
    ...
    float PassiveSphericalRange = 80;
    ...
    float ProjectileMaxDistance = 2000;

    float ProjectileVelocity = 2000f;
    ...
    float RepulseSphericalRange = 400;
    ...
    float TurretProjectileVelocity = 500f;
    ...
    void RunUpdate()
    {
        ...
        if (HasTarget && switches.DoAim)
        {
            ...
            ReferenceToLeadNormalized =
                ReferenceToLead / TargetDistance;
    ...
    void UpdateDerivative()
    {
        ...
        var newKD = Math.Pow(dot, DerivativeNonLinearity);
...
public static class AutoDodge
{
    ...
    public static Vector3D Check(MyDetectedEntityInfo theirGrid, Vector3D theirPos, Vector3D ourPos, Vector3D ourVelocity, IMyCubeGrid ourGrid, Vector3D ourAngularVelocity)
    {
        ...
        Vector3D theirClosestLeadPoint = Vector3D.Zero;
        Vector3D ourClostestLeadPoint = Vector3D.Zero;
    ...
    private const double outlierThresholdMultiplier = 4.0;
...
internal class GravityDriveManager
{
    ...
    private readonly int massBlockBalanceDelay = 301;
    ...
    GravityDriveManagerState
        previousState =
            GravityDriveManagerState.ActiveMove;
    ...
    IMyShipController ShipController;
    double shipMass;
    private readonly int spaceBallBalanceDelay = 1201;
    ...
    void BalanceSpaceBalls(List<MassBlock> blocks, List<SpaceBall> balls, Vector3D centerOfMass)
    {
        ...
        foreach (var ball in spaceBalls)
            if (totalMoment.ALengthSquared() > 10 * 10)
            {
                ...
                var upMomentLength =
                    upAdjustedMoment.ALengthSquared() - 2 * 2;
...
public class Guns
{
    ...
    public Vector3D GetAimingReferencePos(Vector3D fallback)
    {
        ...
        var lowestTimeToFire = GetLowestTimeToFire();
        var averagePos = Vector3D.Zero;
...
internal class KratosMissile
{
    ...
    {
        var forwardThrusters2 = forwardThrusters;
    ...
    Vector3D _savedTargetVelocity = Vector3D.Zero;
...
internal class KratosMissileManager
{
    ...
    void LoiterAndPursue(Vector3D gravity, Vector3D gravityNormal)
    {
        foreach (var missile in _launchedMissiles)
        {
            ...
            if (target == ArgusTargetableShip.Default || !_attack)
                missile.LoiterAroundHost();
            else
                missile.InterceptTarget();
            missile.Update(gravity, gravityNormal);
    ...
    {
        var theta = 2 * Math.PI * _rng.NextDouble();
        var phi = Math.Acos(2 * _rng.NextDouble() - 1);
...
internal class MassBlock
{
    ...
    public void CalculateMoment(Vector3D centerOfMass)
    {
        ...
        double mass = Functional ? 50000 : 0;
