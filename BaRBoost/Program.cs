...
namespace IngameScript
{
    public partial class Program : MyGridProgram
    {
        ...
        private RepairSystemHandler _handler;
        
        public Program()
        {
            _handler = InitBAR();
            Echo(_handler.Count.ToString());
            _handler.ScriptControlled = true;
        ...
        HashSet<IMyCubeGrid> _discoveredEnemyGrids = new HashSet<IMyCubeGrid>();
        ...
        public void Main(string argument, UpdateType updateSource)
        {
            ...
            // _target++;
            //
            // var possibleTargets = _handler.PossibleTargets();
            // if (possibleTargets.Count <= 0) return;
            // if (_target >= possibleTargets.Count)
            // {
            //     _target = 0;
            ...
            //
            //
            // var target = possibleTargets[_target];
            // Echo((target.FatBlock != null).ToString());
            //
            // if (target.FatBlock == null)
            // {
            //     Echo(target.BuildLevelRatio.ToString());
            // }
            // // Echo(target.FatBlock.DefinitionDisplayNameText);
            // // Echo(target.FatBlock.EntityId.ToString());
            //
            //
            // // if (target.HasDeformation || !target.IsFullIntegrity || target.FatBlock == null || target.FatBlock.IsFunctional == false)
            // // {            
            //     _handler.CurrentPickedTarget = target;
            //
            //     
            // //}
            _handler.AreaWidth = float.MaxValue;
            _handler.AreaDepth = float.MaxValue;
            _handler.AreaHeight = float.MaxValue;
            
            Echo("IsUpdating");
            Echo(_handler.Count.ToString());
            var grindables = _handler.PossibleGrindTargets();

            foreach (var grindable in grindables)
            {
                if (grindable.FatBlock != null)
                    Echo(grindable.FatBlock.DisplayNameText);
                var grid = grindable.CubeGrid;
                if (grid != null && !grid.Closed)
                {
                    _discoveredEnemyGrids.Add(grid);
                }
            }



            foreach (var grid in _discoveredEnemyGrids)
            {
                Echo("Grid: " + grid.CustomName);
                var min = grid.Min;
                var max = grid.Max;
                for (int x = min.X; x <= max.X; x++)
                {
                    for (int y = min.Y; y <= max.Y; y++)
                    {
                        for (int z = min.Z; z <= max.Z; z++)
                        {
                            ...
                            var warhead = fatBlock as IMyWarhead;
                            if (warhead != null)
                            {
        ...
        private RepairSystemHandler InitBAR()
        {
            var auto = new RepairSystemHandler();
            auto.Init(GridTerminalSystem, (IMyShipWelder blk) =>
            {
                try
                {
